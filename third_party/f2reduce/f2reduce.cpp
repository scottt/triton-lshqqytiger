...
memxor_lop7(uint64_t *RESTRICT dst, const uint64_t *RESTRICT src1,
            const uint64_t *RESTRICT src2, const uint64_t *RESTRICT src3,
            const uint64_t *RESTRICT src4, const uint64_t *RESTRICT src5,
...
static void kronrod(uint64_t *RESTRICT matrix, uint64_t rows, uint64_t stride,
                    const uint64_t *RESTRICT workspace,
                    uint64_t *RESTRICT cache, const uint64_t *RESTRICT pivots,
                    int k) {
  ...
  for (int o = 0; o < 6; o++) {
    ...
    for (int j = 0; j < subkays[o]; j++) {
      uint64_t p = (1ull << j);
      memcpy(subcache + (p << logwidth),
             matrix + pivots[j + cumkays[o]] * stride, N * 8);
      for (uint64_t i = 1; i < p; i++) {
        memxor_inplace<N>(subcache + ((i + p) << logwidth),
                          subcache + (i << logwidth),
                          subcache + (p << logwidth));
      }
    }
  ...
  for (uint64_t r = 0; r < rows; r++) {
    ...
    if (k <= 32) {
      memxor_lop5<N>(matrix + r * stride, caches[0] + (w0 << logwidth),
                     caches[1] + (w1 << logwidth), caches[2] + (w2 << logwidth),
                     caches[3] + (w3 << logwidth));
    } else {
      uint64_t w4 = (w >> cumkays[4]) & mask4;
      uint64_t w5 = (w >> cumkays[5]) & mask5;
      memxor_lop7<N>(matrix + r * stride, caches[0] + (w0 << logwidth),
                     caches[1] + (w1 << logwidth), caches[2] + (w2 << logwidth),
                     caches[3] + (w3 << logwidth), caches[4] + (w4 << logwidth),
                     caches[5] + (w5 << logwidth));
    }
  }
}

bool find_pivots(uint64_t *RESTRICT pivots, uint64_t *RESTRICT this_strip,
                 uint64_t rows, uint64_t &starting_row, uint64_t *workspace,
                 uint64_t &next_b, uint64_t final_b, int K, int &k) {

  static bool find_pivots(uint64_t *RESTRICT pivots,
                          uint64_t *RESTRICT this_strip, uint64_t rows,
                          uint64_t &starting_row, uint64_t *workspace,
                          uint64_t &next_b, uint64_t final_b, int K, int &k) {
   ...
   * value of the chunk width N, which outperforms having a variable-length
   * loop.
   */
  static void chunked_kronrod(
      const uint64_t *RESTRICT pivots, uint64_t *RESTRICT matrix, uint64_t rows,
      uint64_t strips, uint64_t stride, const uint64_t *workspace,
      uint64_t *RESTRICT cache, int k) {

    uint64_t re = strips - 1;

#define KRONROD(N)                                                             \
  kronrod<N>(matrix + (strips - re), rows, stride, workspace, cache, pivots, k)
...
#undef KRONROD
   ...
   */
  static bool perform_K_steps(
      uint64_t *RESTRICT matrix, uint64_t *RESTRICT stripspace, uint64_t rows,
      uint64_t strips, uint64_t stride, uint64_t &starting_row,
      uint64_t *workspace, uint64_t *RESTRICT cache, uint64_t &next_b, int K,
      uint64_t final_b) {
    ...
    int k = 0;
    bool completed_strip = find_pivots(pivots, stripspace, rows, starting_row,
                                       workspace, next_b, final_b, K, k);
    ...
    for (int i = 0; i < k; i++) {
      if (pivots[i] != starting_row) {
        // swap rows in matrix:
        swap_rows(matrix + starting_row * stride, matrix + pivots[i] * stride,
                  strips);
  ...
  void inplace_rref_strided_K(uint64_t *RESTRICT matrix,
                              uint64_t *RESTRICT stripspace, uint64_t rows,
                              uint64_t cols, uint64_t stride,
                              uint64_t *workspace, uint64_t *cache, int K) {
    ...
    for (uint64_t current_strip = 0; current_strip < strips; current_strip++) {
      ...
      while (current_row < rows) {
        if (perform_K_steps(offset_matrix, stripspace, rows,
                            strips - current_strip, stride, current_row,
                            workspace, cache, next_b, K, final_b)) {
          break;
        }
      }
  ...
  void inplace_rref_strided_heap(uint64_t *matrix, uint64_t rows, uint64_t cols,
                                 uint64_t stride, int K) {

    static void inplace_rref_strided_K(
        uint64_t *RESTRICT matrix, uint64_t *RESTRICT stripspace, uint64_t rows,
        uint64_t cols, uint64_t stride, uint64_t *workspace, uint64_t *cache,
        int K) {
      ...
      void *cache_raw = malloc(
          256 * (1 + (1 << subkays[0]) + (1 << subkays[1]) + (1 << subkays[2]) +
                 (1 << subkays[3]) + (1 << subkays[4]) + (1 << subkays[5])));
      ...
      inplace_rref_strided_K(matrix, stripspace, rows, cols, stride, workspace,
                             cache, K);
  ...
  static void inplace_rref_strided_heap(uint64_t *matrix, uint64_t rows,
                                        uint64_t cols, uint64_t stride, int K) {
    ...
    // Array for storing 256-byte chunks of linear combinations of pivot rows:
    void *cache_raw = malloc(256 * (1 + (1 << subkays[0]) + (1 << subkays[1]) +
                                    (1 << subkays[2]) + (1 << subkays[3]) +
                                    (1 << subkays[4]) + (1 << subkays[5])));
  ...
  static void inplace_rref_small(uint64_t *matrix, uint64_t rows,
                                 uint64_t cols) {
...
void inplace_rref_strided(uint64_t *matrix, uint64_t rows, uint64_t cols,
  ...
  if ((rows <= 64) && (cols <= 64)) {
    ...
    for (uint64_t i = 0; i < rows; i++) {
      matrix2[i] = matrix[i * stride];
    }
    inplace_rref_small(matrix2, rows, cols);
    for (uint64_t i = 0; i < rows; i++) {
      matrix[i * stride] = matrix2[i];
    }
  } else {
